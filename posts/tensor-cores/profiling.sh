...
echo "method,size,time,shared,flops,tflops,powerBefore,powerAfter,temperatureBefore,temperatureAfter,clockBefore,clockAfter,perf" > $dir/profile.csv
...
for i in 16; do
  ...
  for size in $(seq 256 256 16384); do
    for iteration in $(seq 1 1 30); do
